def get_verbose(field_list, model):
    result = []
    for field in field_list:
        if hasattr(getattr(model, field), 'field'):
            verbose = getattr(model, field).field.verbose_name
        else:
            verbose = getattr(model, f'{field}_verbose')
        
        result.append(verbose)

    return result

# def get_queryset_value(queryset, list_fields):
#     rows = []
#     for obj in queryset:
#         data = []
#         for field, property_ in list_fields.items():
#             property_.setdefault('format', None)
            
#             # choice field일 경우 display name 가져옴            
#             if property_.get('get_display'):
#                 data.append((getattr(obj, f'get_{field}_display'), property_))
#             else:
#                 data.append((getattr(obj, field), property_))
        
#         rows.append({'pk': obj.pk, 'data': data})
...
# except ImportError:
#     from django.utils.translation import gettext_lazy as _  # Django V4
