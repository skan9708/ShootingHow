...
export default function BookingOrder(props) {
    ...
    const handleBookingSubmit = () => {
        ...
        setTimeout(() => {
            // 현재 페이지의 id를 사용하여 complete 페이지로 이동
            router.push(`/booking/complete/${router.query.id}`); 
            setIsPageLoading(false); // 실제로는 페이지 이동 후 컴포넌트가 언마운트될 수 있음
        }, 1000);
    };

    const tossPay = async () => {
        // ... (기존 tossPay 함수 로직은 유지) ...
        // 이 함수 내에서 실제 예약 성공 후 페이지 이동이 있다면, 해당 부분도 handleBookingSubmit을 사용하거나 유사한 로직으로 변경 고려
        // 지금은 우선적으로 "예약하기" 버튼에 직접 로딩을 연결합니다.
        // 예시: 예약 성공 콜백 내에서 handleBookingSubmit() 호출 또는 아래의 직접 버튼 로직 사용

        // 임시로 TOSS 결제 버튼이 아닌 일반 "예약하기" 버튼에 로딩을 연결합니다.
        // 실제 TOSS 연동 시에는 TOSS SDK의 성공 콜백에서 페이지 이동 및 로딩 상태 관리가 필요합니다.
        handleBookingSubmit(); // TossPay 대신 직접 호출하도록 연결 (요청에 따라)
    };

    if (!facilityInfo) { // facilityInfo가 없을 경우 (예: id가 유효하지 않음)
        return <div>유효하지 않은 접근입니다.</div>; // 또는 다른 적절한 UI
    }
                ...
                <Section>
                    <Container>
                        ...
                        <Center style={{ marginTop: 50 }}>
                            <Button onClick={handleBookingSubmit}>예약하기</Button>
                        </Center>
